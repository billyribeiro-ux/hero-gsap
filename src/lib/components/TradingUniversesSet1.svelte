<script lang="ts">
	...
	function initSpread() {
		...
		let t = 0, active = false, collision = false; const bids: { x: number; y: number; size: number; speed: number }[] = [], asks: { x: number; y: number; size: number; speed: number }[] = [];
		...
		function draw() {
			...
			const midX = w / 2 + Math.sin(t * 0.5) * w * 0.1; collision = false;
			x1.fillStyle = 'rgba(0,255,136,0.6)'; bids.forEach(b => { b.x += b.speed; if (b.x > midX - 20) { b.x = midX - 20; collision = true; } x1.beginPath(); x1.arc(b.x, b.y, b.size, 0, Math.PI * 2); x1.fill(); });
			x1.fillStyle = 'rgba(255,51,68,0.6)'; asks.forEach(a => { a.x -= a.speed; if (a.x < midX + 20) { a.x = midX + 20; collision = true; } x1.beginPath(); x1.arc(a.x, a.y, a.size, 0, Math.PI * 2); x1.fill(); });
			if (collision) { x1.strokeStyle = `rgba(255,255,255,${0.3 + Math.sin(t * 20) * 0.2})`; x1.lineWidth = 2; x1.beginPath(); x1.moveTo(midX, 0); x1.lineTo(midX, h); x1.stroke(); }
			x2.clearRect(0, 0, w, h); x2.fillStyle = 'rgba(0,255,136,0.3)'; x2.font = 'bold 24px "JetBrains Mono"'; x2.textAlign = 'right'; x2.fillText('BID', midX - 20, h / 2); x2.fillStyle = 'rgba(255,51,68,0.3)'; x2.textAlign = 'left'; x2.fillText('ASK', midX + 20, h / 2); x2.textAlign = 'start';
			rafIds.push(requestAnimationFrame(draw));
		}
		rafIds.push(requestAnimationFrame(draw));
		const trigger = ScrollTrigger.create({ trigger: '#h5', start: 'top 70%', onEnter: () => { active = true; const tl = gsap.timeline({ delay: 0.3 }); tl.to('#h5f', { opacity: 0.6, duration: 0.03 }, 0.5); tl.to('#h5f', { opacity: 0, duration: 1.5 }, 0.53); tl.to('#h5t', { opacity: 1, duration: 0.6, ease: 'power4.out' }, 0.8); tl.add(() => shake(document.getElementById('h5-content')!, 25, 0.4), 0.8); tl.to('#h5s', { opacity: 1, duration: 0.8 }, 1.5); } }); triggers.push(trigger);
	}

	onMount(() => { mx = window.innerWidth / 2; my = window.innerHeight / 2; prevMx = mx; prevMy = my; rafIds.push(requestAnimationFrame(updateMouseVelocity)); initFloor(); initAlgorithm(); initDarkPool(); initMomentum(); initSpread(); });

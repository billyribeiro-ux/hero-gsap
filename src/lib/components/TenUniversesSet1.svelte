<script lang="ts">
	...
	let mx = 0;
	let my = 0;
	let c1: HTMLCanvasElement, c2: HTMLCanvasElement, c3: HTMLCanvasElement, c4: HTMLCanvasElement, c5: HTMLCanvasElement;
	...
	function initSupernova() {
		...
		for (let i = 0; i < 800; i++) {
			stars.push({ x: Math.random() * 2 - 1, y: Math.random() * 2 - 1, z: Math.random(), size: Math.random() * 1.5 + 0.3 });
		}
		for (let i = 0; i < 60; i++) {
			gas.push({ x: (Math.random() - 0.5) * w, y: (Math.random() - 0.5) * h, r: Math.random() * 150 + 50, vx: (Math.random() - 0.5) * 0.3, vy: (Math.random() - 0.5) * 0.3, hue: Math.random() * 60 - 10, alpha: 0 });
		}
		...
		function draw() {
			...
			t += 0.005;
			x.fillStyle = 'rgba(2,0,5,0.15)'; x.fillRect(0, 0, w, h);
		...
		const trigger = ScrollTrigger.create({
			trigger: '#s1', start: 'top 80%',
			onEnter: () => {
				active = true; const tl = gsap.timeline({ delay: 0.3 });
				tl.add(() => { exploded = true; }, 1);
				tl.fromTo('.s1-vignette', { background: 'radial-gradient(ellipse,rgba(255,200,100,0.8) 0%,transparent 60%)' }, { background: 'radial-gradient(ellipse,transparent 30%,rgba(0,0,5,0.8) 100%)', duration: 1.5, ease: 'power3.out' }, 1);
	...
	function initNeural() {
		...
		function firePulse() {
			if (!active) return; const src = Math.floor(Math.random() * nodes.length), n = nodes[src]; if (n.connections.length === 0) return;
			const tgt = n.connections[Math.floor(Math.random() * n.connections.length)]; pulses.push({ from: src, to: tgt, progress: 0, speed: 0.02 + Math.random() * 0.03 }); nodes[src].pulse = 1;
	...
	onMount(() => {
		...
		initSupernova(); initNeural(); initAbyss(); initStorm(); initHeartbeat();
	});
